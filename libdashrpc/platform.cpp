...
namespace dashrpc
{
    ...
    {
        connection->channel =
            grpc::CreateChannel(fmt::format("{}:{}", address, port), grpc::InsecureChannelCredentials());
    ...
    void PrintMetadataMsg(auto metaDataRes)
    {
    ...
    void PrintProofMsg(auto proofRes)
    ...
    int32_t platform::wait_for_state_transition_result(void)
    {
        ...
        obj["proof"] = {
            {"rootTreeProof", waitStateRes.proof().signature_llmq_hash()},
            {"storeTreeProof",
